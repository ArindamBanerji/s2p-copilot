...
def test_payment_strategy_returns_analyzed_suppliers():
    seed_supplier()
    data = assert_dict_response(client.get("/api/s2p/suppliers/payment-strategy"))

    assert isinstance(data["strategies"], list)
    assert data["suppliers_analyzed"] == len(data["strategies"])


def test_payment_behavior_returns_known_supplier_strategy():
    seed_supplier("SUP-001")
    data = assert_dict_response(client.get("/api/s2p/suppliers/payment-behavior", params={"supplier_id": "SUP-001"}))

    assert data["supplier_id"] == "SUP-001"
    assert data["recommended_strategy"] in {"early_pay", "on_time", "extend"}


def test_payment_behavior_unknown_supplier_returns_404():
    seed_supplier("SUP-001")
