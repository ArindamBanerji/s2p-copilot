...
def test_early_warning_list_returns_valid_payload():
    seed_supplier()
    data = assert_dict_response(client.get("/api/s2p/suppliers/early-warnings"))

    assert isinstance(data["warnings"], list)
    assert data["monitored_suppliers"] >= 1


def test_early_warning_trends_returns_supplier_rows():
    seed_supplier()
    data = assert_dict_response(client.get("/api/s2p/suppliers/trends"))

    assert isinstance(data["suppliers"], list)
    assert data["total"] == len(data["trends"])


def test_early_warning_trend_signals_returns_known_supplier():
    seed_supplier("SUP-001")
    data = assert_dict_response(client.get("/api/s2p/suppliers/trend-signals", params={"supplier_id": "SUP-001"}))

    assert data["supplier_id"] == "SUP-001"
    assert isinstance(data["signals"], list)


def test_early_warning_trend_signals_unknown_supplier_returns_404():
    seed_supplier("SUP-001")
